...
bool Layby::process(Tracking &track, Mat &image, vector<PredictResult> predict)
{
 ...
 *
 */
void Layby::drawImage(Tracking track, Mat &image)
{
    // 赛道边缘
    for (size_t i = 0; i < track.pointsEdgeLeft.size(); i++)
    {
 ...
 * @param left
 */
void Layby::curtailTracking(Tracking &track, bool left)
{
    if (left) // 向左侧缩进
    {
